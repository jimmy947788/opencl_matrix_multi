...
#define CL_KERNEL_NAME "vector_multi_matrix"
...
int main(int argc, char** argv)
{
    ...
    a = (cl_double *)malloc(sizeof(cl_double) * matrix_width);
    for(int i=0; i<= matrix_width -1; i++)
    {
        a[i] = rand() & 1;
    }   
    
    b = (cl_double *)malloc(sizeof(cl_double) * matrix_width * matrix_height);
    //printf("b = { ");
    for(int j=0; j<= matrix_height -1; j++)
    {
        for(int i=0; i<= matrix_width-1; i++) 
        {
            ...
            b[index] = randfrom(0, 1500);
            //printf("%.2f, ", index, b[index]);
        }
        //printf("\n ");
    }
    //printf("}\n ");
    ...
    a_buffer = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, sizeof(cl_double) * matrix_width, a, &errNum); 
    if(errNum < 0) {
      perror("Couldn't create a buffer");
      exit(1);   
    };
    b_buffer = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, sizeof(cl_double) * matrix_width * matrix_height, b, &errNum);    
    if(errNum < 0) {
      perror("Couldn't create a buffer");
      exit(1);   
    };
    c_buffer= clCreateBuffer(context,CL_MEM_WRITE_ONLY ,sizeof(cl_double)* matrix_width, NULL, &errNum);
     if(errNum < 0) {
      perror("Couldn't create a buffer");
      exit(1);   
    };
    ...
    errNum = clSetKernelArg(kernel, 3, sizeof(int), &matrix_height);
    if(errNum < 0) {
        perror("Couldn't set a kernel argument");
        exit(1);   
    };
    ...
    */
    ...
    const size_t global_size[] = { matrix_width };
    const size_t local_size[] = { 1 };
    errNum = clEnqueueNDRangeKernel(queue, kernel, dim, global_offset, global_size, local_size, 0, NULL, NULL);
    /*
    const size_t global_size[1] = {  matrix_width };
    const size_t local_size[1] = { matrix_height };
    errNum = clEnqueueNDRangeKernel(queue, kernel, 1, NULL, global_size, NULL, 0, NULL, NULL);
    */
    checkErr(errNum, "clEnqueueNDRangeKernel");    
    printf("pass kernel code to GPU%d\n", 0);
    
    c = (cl_double *)malloc(sizeof(cl_double) * matrix_width);
    /* Read and print the result */
    errNum = clEnqueueReadBuffer(queue, c_buffer, CL_TRUE, 0, sizeof(cl_double) * matrix_width, c, 0, NULL, NULL);
    if(errNum < 0) {
